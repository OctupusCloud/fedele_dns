...
from fedele_dns.fields import AddressField
...
# -
import fedele_dns.models.zone as zone
...
class Record(NetBoxModel):
    ...
    def get_absolute_url(self):
        return reverse("plugins:fedele_dns:record", kwargs={"pk": self.id})
    ...
    def validate_name(self):
        ...
        if self.type not in get_plugin_config(
            "fedele_dns", "tolerate_non_rfc1035_types", default=list()
        ):
            try:
                validate_extended_hostname(
                    self.name,
    ...
    def check_unique(self):
        if not get_plugin_config("fedele_dns", "enforce_unique_records", False):
            return
