...
def validate_fqdn(name):
    if get_plugin_config("fedele_dns", "tolerate_underscores_in_hostnames"):
...
def validate_extended_hostname(name, tolerate_leading_underscores=False):
    ...
    elif get_plugin_config("fedele_dns", "tolerate_underscores_in_hostnames"):
        regex = rf"^([*@]|(\*\.)?{TOLERANT_LABEL}(\.{TOLERANT_LABEL})*\.?)$"
    else:
        regex = rf"^([*@]|(\*\.)?{LABEL}(\.{LABEL})*\.?)$"
...
def validate_domain_name(name):
    if name == "." and get_plugin_config("fedele_dns", "enable_root_zones"):
        return
