...
from fedele_dns.utilities.ipam_coupling import (
    ipaddress_cf_data,
    get_address_record,
    new_address_record,
...
def ip_address_check_permissions_save(instance, **kwargs):
    ...
    if not get_plugin_config("fedele_dns", "feature_ipam_coupling"):
        return
    ...
    try:
        if instance.id is None:
            ...
            if record is not None:
                record.full_clean()
                check_permission(request, "fedele_dns.add_record", record)
            ...
            if record is not None:
                ...
                if zone_id is not None:
                    ...
                    record.full_clean()
                    check_permission(request, "fedele_dns.change_record", record)
                else:
                    check_permission(request, "fedele_dns.delete_record", record)
